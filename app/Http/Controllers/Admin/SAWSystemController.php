...
class SAWSystemController extends Controller
{
    ...
    public function search(Request $request){
        ...
        try {
            ...
            $data = $this->SWA_System($request["periode"], $datakuota->kuota);
//            dd($data);
    ...
    public function SWA_System($periode, $kuota){
        try {
...
//        System Normalisasi
            ...
            $datanormalisasi = DB::table("calon_penerima")
                ...
                ->where("periode_penerimaan.id", "=", $periode)
            ...
            $databenefit = DB::table("kriteria_benefit")->where("id_periode", "=", $periode)->get();
...
//        Final Data Ranking
            ...
            $cek = $data->sortByDesc("jumlah");

            //Colections kecil to Besar
            $cek1 = $data->sortBy("jumlah");

            $data_final_ranking = [];

            foreach ($cek as $item){
                $data_final_ranking[] = $item;
            }
//            dd($data_final_ranking);
